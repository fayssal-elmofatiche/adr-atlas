...
describe("Atlas Server API", () => {
  ...
  describe("GET /api/adrs", () => {
    it("returns all ADRs", async () => {
      const res = await request("GET", "/api/adrs");
      expect(res.status).toBe(200);
      expect(res.body).toHaveLength(3);
    });

    it("filters by status", async () => {
      const res = await request("GET", "/api/adrs?status=accepted");
      expect(res.status).toBe(200);
    ...
    it("filters by tag", async () => {
      const res = await request("GET", "/api/adrs?tag=database");
      expect(res.status).toBe(200);
    ...
    it("filters by component", async () => {
      const res = await request("GET", "/api/adrs?component=order-service");
      expect(res.status).toBe(200);
      expect(res.body).toHaveLength(2);
    });
  });
  ...
  describe("GET /api/adrs/:id", () => {
    it("returns ADR detail", async () => {
      const list = await request("GET", "/api/adrs");
      const id = list.body[0].id;

      const res = await request("GET", `/api/adrs/${id}`);
      expect(res.status).toBe(200);
    ...
    it("returns 404 for non-existent ID", async () => {
      const res = await request("GET", "/api/adrs/999");
      expect(res.status).toBe(404);
    });

    it("returns 400 for invalid ID", async () => {
      const res = await request("GET", "/api/adrs/abc");
  ...
  describe("GET /api/adrs/search", () => {
    it("finds ADRs by keyword", async () => {
      const res = await request("GET", "/api/adrs/search?q=Kafka");
      expect(res.status).toBe(200);
      expect(res.body).toHaveLength(1);
    ...
    it("returns 400 when q is missing", async () => {
      const res = await request("GET", "/api/adrs/search");
      expect(res.status).toBe(400);
    });

    it("returns empty for no matches", async () => {
      const res = await request("GET", "/api/adrs/search?q=nonexistent-xyz");
      expect(res.status).toBe(200);
  ...
  describe("GET /api/graph", () => {
    ...
    it("filters by status", async () => {
      const res = await request("GET", "/api/graph?status=proposed");
      expect(res.status).toBe(200);
  ...
  describe("GET /api/components/:name/adrs", () => {
    it("returns ADRs for a component", async () => {
      const res = await request("GET", "/api/components/user-service/adrs");
      expect(res.status).toBe(200);
      expect(res.body.length).toBeGreaterThanOrEqual(1);
    });

    it("returns empty for unknown component", async () => {
      const res = await request("GET", "/api/components/nonexistent/adrs");
      expect(res.status).toBe(200);
      expect(res.body).toHaveLength(0);
    });
  });
